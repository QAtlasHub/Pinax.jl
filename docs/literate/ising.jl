...
#
# **Source:** self-contained — a 2-D Metropolis simulation written with `Plots` + `Random`.
#
# Hamiltonian $H=-J\sum_{\langle ij\rangle} s_i s_j$; flips accepted with probability
# $\min(1, e^{-\beta\Delta E})$. The model orders below the Onsager temperature
# $T_c=2/\ln(1+\sqrt 2)\approx 2.269$ and disorders above it.
...
Pinax.reset!(; title="2-D Ising model — Monte Carlo")
